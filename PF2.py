...
parser.add_argument('--task_name', type=str, default='PF2',
...
'''
cand_list = list(graph.edge_list['field']['paper']['PFL2'].keys())
'''
...
vdata=dill.Unpickler(open(args.data_dir + '/PF2_valid%s.pkl' % args.dataset,'rb')).load()

vyl=np.array(vdata[5].todense().tolist())
tdata=dill.Unpickler(open(args.data_dir + '/PF2_test%s.pkl' % args.dataset,'rb')).load()
tyl=[]
for slice in tdata:
    tyl+=[np.array(slice[5].todense().tolist())]


feature,idl=feature_cal(graph)
data = dill.Unpickler(open(args.output_dir + '/sample%s_%s_%s.pkl' % ('PF2', args.dataset,pin), 'rb')).load().train_data
    ...
    for _ in range(args.repeat):
        if _ ==0:
            for paper_gh,author_gh,total_gh,edge_list,x_ids,ylabel in train_data:
                ...
                ylabel=np.array(ylabel.todense().tolist())
                y_temp+=[ylabel]
            ...
            for idx,(paper_gh,author_gh,total_gh,edge_list,x_ids,ylabel) in enumerate(train_data) :
                paper_gh=feature_extract(paper_gh,'paper',feature,idl)
                author_gh=feature_extract(author_gh,'author',feature,idl)
