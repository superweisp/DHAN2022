...
cand_list = list(graph.edge_list['venue']['paper']['PVJournal'].keys())
...
'''
for target_id in graph.edge_list['paper']['venue']['PVJournal']:
    for source_id in graph.edge_list['paper']['venue']['PVJournal'][target_id]:
        _time = graph.edge_list['paper']['venue']['PVJournal'][target_id][source_id]
...
if not os.path.exists(args.data_dir + '/PV_valid%s.pkl' % args.dataset):
    tdata=[]
    vdata=node_classification_sample(randint(),valid_pairs,valid_range)
    dill.dump(vdata, open(args.data_dir + '/PV_valid%s.pkl' % args.dataset, 'wb'))
    print('vdata has been saved')
if not os.path.exists(args.data_dir + '/PV_test%s.pkl' % args.dataset):
    tdata=[]
    for _ in range(10):
        tdata+=[node_classification_sample(randint(),test_pairs,test_range)]
    dill.dump(tdata, open(args.data_dir + '/PV_test%s.pkl' % args.dataset, 'wb'))
