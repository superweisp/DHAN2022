...
'''
for target_id in graph.edge_list['paper']['field']['PFL2']:
    for source_id in graph.edge_list['paper']['field']['PFL2'][target_id]:
        _time = graph.edge_list['paper']['field']['PFL2'][target_id][source_id]
...
if not os.path.exists(args.data_dir + '/PF2_valid%s.pkl' % args.dataset):
    tdata=[]
    vdata=node_classification_sample(randint(),valid_pairs,valid_range)
    dill.dump(vdata, open(args.data_dir + '/PF2_valid%s.pkl' % args.dataset, 'wb'))
    print('vdata has been saved')
if not os.path.exists(args.data_dir + '/PF2_test%s.pkl' % args.dataset):
    tdata=[]
    for _ in range(10):
        tdata+=[node_classification_sample(randint(),test_pairs,test_range)]
    dill.dump(tdata, open(args.data_dir + '/PF2_test%s.pkl' % args.dataset, 'wb'))
