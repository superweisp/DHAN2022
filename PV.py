...
pin=1
rd=0
data=dill.Unpickler(open(args.output_dir + '/sample%s_%s_%s.pkl' % ('PV',args.dataset,pin),'rb')).load()
tdata=dill.Unpickler(open(args.data_dir + '/PV_test%s.pkl' % args.dataset,'rb')).load()
vdata=dill.Unpickler(open(args.data_dir + '/PV_valid%s.pkl' % args.dataset,'rb')).load()
best_loss=1000
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    train_data = data.train_data[rd]
    valid_data=data.valid_data[rd]
    rd+=1
    ...
    with torch.no_grad():
        ...
        if valid_ndcg > best_val and loss<=best_loss:
            best_loss=loss
            best_val = valid_ndcg
